...
def parse_record_types(record_type):
    if record_type is None:
        return []
    return [item.strip() for item in record_type.split(',') if item.strip()]


def merge_target_records_from_chunks(chunk_records_list, record_types):
    merged_records = {key: {} for key in record_types}

    for chunk_records in chunk_records_list:
        for record_key in record_types:
            for module_name, payload in chunk_records.get(record_key, {}).items():
                merged_records[record_key].setdefault(module_name, []).append(payload[0])

    helper_processor = AttnProcessor(record_type=','.join(record_types))
    for record_key in record_types:
        for module_name, value_chunks in merged_records[record_key].items():
            merged_value = torch.cat(value_chunks, dim=0)
            if merged_value.shape[0] // 16 == 1:
                merged_records[record_key][module_name] = [merged_value, None, None]
            else:
                project_matrix, basis_ortho = helper_processor.cal_gram_schmidt(merged_value)
                merged_records[record_key][module_name] = [merged_value, project_matrix, basis_ortho]

    return merged_records


def build_target_records_in_chunks(unet, scheduler, target_concept_encoding, uncond_encoding, guidance_scale, record_type, target_record_batch_size):
    record_types = parse_record_types(record_type)
    if len(record_types) == 0:
        return {}

    num_concepts = len(target_concept_encoding)
    if target_record_batch_size <= 0:
        chunk_size = num_concepts
    else:
        chunk_size = min(target_record_batch_size, num_concepts)

    chunk_records_list = []
    for start in range(0, num_concepts, chunk_size):
        end = min(start + chunk_size, num_concepts)
        chunk_encoding = target_concept_encoding[start:end]
        chunk_len = len(chunk_encoding)

        _, chunk_records = diffusion(
            unet=unet,
            scheduler=scheduler,
            latents=torch.zeros(chunk_len, 4, 64, 64).to(unet.device, dtype=target_concept_encoding.dtype),
            text_embeddings=torch.cat([uncond_encoding] * chunk_len + [chunk_encoding], dim=0),
            total_timesteps=1,
            start_timesteps=0,
            guidance_scale=guidance_scale,
            record=True,
            record_type=record_type,
            desc=f"Calculating target records [{start}:{end}]",
        )
        chunk_records_list.append(chunk_records)

        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    return merge_target_records_from_chunks(chunk_records_list, record_types)
...
def main():
    ...
    parser.add_argument('--target_record_batch_size', type=int, default=16,
                        help='Concept chunk size for building target records. Use smaller values to reduce peak VRAM.')
    ...
    if 'erase' in mode_list or 'retain' in mode_list:
        ...
        target_records = build_target_records_in_chunks(
            unet=unet,
            scheduler=pipe.scheduler,
            target_concept_encoding=target_concept_encoding,
            uncond_encoding=uncond_encoding,
            guidance_scale=args.guidance_scale,
            record_type=args.record_type,
            target_record_batch_size=args.target_record_batch_size,
        )
        pipe.scheduler.set_timesteps(args.total_timesteps)
        for record_key in parse_record_types(args.record_type):
            original_keys = list(target_records[record_key].keys())
            target_records[record_key].update({
                f"{timestep}.{'.'.join(key.split('.')[1:])}": target_records[record_key][key]
